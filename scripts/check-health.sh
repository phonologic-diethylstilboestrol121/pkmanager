...
# Usage:
#   ./scripts/check-health.sh          # Full check (API + diagnostics + smoke tests)
#   ./scripts/check-health.sh --quick  # Quick check (API + diagnostics only)
...
API_BASE="${PKM_API_URL:-http://localhost:${SERVER_HTTP_PORT:-5000}}"
QUICK_MODE=false
...
# TCP connection test (PostgreSQL must be listening on TCP port 5432)
PGDATA_DIR="${PGDATA:-$PROJECT_DIR/data/pgdata}"
if [[ -d "$PGDATA_DIR" ]]; then
  echo -e "  $PASS  PostgreSQL 数据目录存在: $PGDATA_DIR"
  if command -v psql &>/dev/null; then
    if PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "SELECT 1" &>/dev/null 2>&1; then
      echo -e "  $PASS  数据库 TCP 连接正常 (${DB_HOST}:${DB_PORT})"
    else
      echo -e "  $WARN  数据库 TCP 连接失败 (可能未启动、未开启 TCP 监听或凭据变更)"
    fi
  else
